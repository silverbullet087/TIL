...
-	모든 함수가 2,3,4 줄로 만들어라. 각 함수가 이야기 하나를 표현했다.
...
-	To 문단을 읽듯이 프로그램이 읽혀야한다. > To 입력 타입에 따라 입력 타입에 값을 일괄 수정한다. >> To 입력 타입이 올바른지 체크한다. >> To 입력 타입에 맞는 일괄 수정값을 세팅한다. >> To 일괄 수정을 DB에 수정한다.
